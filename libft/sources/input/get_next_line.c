...
/*   Updated: 2014/03/11 01:05:59 by kube             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_read		*ft_newread(int file)
{
	t_read			*red;
	void			*tmp;
	int				ret;
	...
	if ((ret = read(file, tmp, GNL_BUFF_SIZE)) < 0)
	{
		free(red);
		free(tmp);
		return (NULL);
...
static	int			ft_findendl(int file, t_read *red)
{
	int				index;
	int				size;
	t_read			*tmplst;
...
int					get_next_line(int file, char **line)
{
	...
	prevtmp = NULL;
	if (!start)
		start = ft_newread(file);
	red = start;
	if (!red || !start)
		return (-1);
	tab[0] = red;
	tab[1] = prevtmp;
	return (ft_print(ft_findendl(file, red), tab, &start, line));
}
